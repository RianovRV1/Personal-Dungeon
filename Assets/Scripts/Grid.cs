...
public class Grid : MonoBehaviour {
    ...
    void GenerateGrid()//user made function to initialize grid
    {
        ...
        Vector3 bottomLeft = transform.position - Vector3.right * gridWorldSize.x / 2 - Vector3.forward * gridWorldSize.y / 2; //Math to take the center position then find the bottomleft location of the grid
        for (int y = 0; y < gridSizeX; y++)
        {
            for (int x = 0; x < gridSizeX; x++)
            {
                Vector3 worldPoint = bottomLeft + Vector3.right * (x * nodeDiameter + nodeRadius) + Vector3.forward * (y * nodeDiameter + nodeRadius); //math that starts at the bottomleft then adds vectors to get the nodes current location
                bool Wall = false;
    ...
    internal Node NodeFromWorldPosition(Vector3 worldPosition)
    {
        ...
        float ypoint = ((worldPosition.z + gridWorldSize.y / 2) / gridWorldSize.y); // gets y position of grid node
